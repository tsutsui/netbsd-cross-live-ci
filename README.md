...
| `machine`      | Yes      |                 | The architecture of the NetBSD environment (currently `vax`, `i386` and `alpha` are supported). |
...
2. **Emulator Setup**: simh is used to run the NetBSD/vax live image, and qemu is used to run NetBSD/i386. To run NetBSD/alpha, patched qemu-system-alpha binary is built during CI.
